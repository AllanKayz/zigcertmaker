...
/**
 * @description Saves a certificate to the history in local storage.
 * @param {string} member - The name of the member.
 * @param {string} category - The membership category.
 * @param {string} membershipNumber - The membership number.
 */
function saveCertificateToHistory(member, category, membershipNumber) {
    if (!member || !category || !membershipNumber) return;
...
/**
 * @description Loads the certificate history from local storage and displays it in the UI.
 */
function loadCertificateHistory() {
  ...
  if (history.length === 0) {
    historyList.innerHTML = "<p>No certificates generated yet.</p>";
    return;
  }
  history.forEach((entry, index) => {
    ...
    item.innerHTML = `
      <div class="history-item-info">
        <div class="history-item-name">${entry.member}</div>
        <div class="history-item-date">Generated on: ${new Date(entry.date).toLocaleDateString()}</div>
      </div>
      <div class="history-item-actions">
        <button class="btn btn-secondary btn-sm view-btn" data-index="${index}">View</button>
  ...
  document.querySelectorAll(".view-btn").forEach((button) => {
    button.addEventListener("click", (e) => {
      ...
      viewCertificateFromHistory(index);
    });
  });
  ...
  document.querySelectorAll(".delete-btn").forEach((button) => {
    button.addEventListener("click", (e) => {
      const index = e.target.getAttribute("data-index");
      deleteCertificateFromHistory(index);
    });
  });
}

/**
 * @description Loads a certificate from history into the main view.
 * @param {number} index - The index of the certificate in the history.
 */
function viewCertificateFromHistory(index) {
  const history = JSON.parse(localStorage.getItem("certificateHistory")) || [];
  const entry = history[index];
  if (entry) {
    memberNameInput.value = entry.member;
    membershipCategoryInput.value = entry.category.split(" ")[3];
    membershipIDInput.value = entry.membershipNumber.replace("ZIG", "");
    member = entry.member;
    category = entry.category;
    membershipNumber = entry.membershipNumber;
    document.querySelector('div[data-view="create"]').click();
  }
}

/**
 * @description Deletes a certificate from the history.
 * @param {number} index - The index of the certificate in the history.
 */
function deleteCertificateFromHistory(index) {
    const history = JSON.parse(localStorage.getItem("certificateHistory")) || [];
